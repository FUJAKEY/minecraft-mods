...
public class ThermalGeneratorTileEntity extends GeneratorTileEntity {
    ...
    public int getEfficiency() {
        ...
        for (BlockPos p : BlockPos.betweenClosed(worldPosition.offset(-1, -1, -1), worldPosition.offset(1, 1, 1))) {
            ...
            if (state.getBlock() == Blocks.LAVA || state.getFluidState().getType() == Fluids.LAVA) {
                nearLava = true;
            } else if (state.getBlock() == Blocks.MAGMA_BLOCK || state.getBlock() == Blocks.FIRE) {
                nearHotBlock = true;
            }
